    ...
    def __init__(
        ...
        if not skip_memory:
            try:
                ...
                # Default to holographic if no provider is configured.
                # Holographic requires no external dependencies (API keys, accounts),
                # making it the best default for new users.
                if not _mem_provider_name:
                    from plugins.memory.holographic import HolographicMemoryProvider
                    if HolographicMemoryProvider().is_available():
                        _mem_provider_name = "holographic"
